...
parser.add_argument(
    "--classifier", help="classifier", type=str, default="dumy_classifier"
)
parser.add_argument(
    "--dataset_path", help="dataset path", type=str, default="../hw2_data/p1/"
)
...
def main():
    ...
    accuracy = float(
        len([x for x in zip(test_labels, predicted_categories) if x[0] == x[1]])
...
def build_confusion_mtx(test_labels_ids, predicted_categories, abbr_categories):
    ...
    plot_confusion_matrix(
        cm_normalized, abbr_categories, title="Normalized confusion matrix"
    )
